...
task('populate-descriptor', 'Populates the descriptor with color palettes and Word parts')
  .addOptionalParam(
    'nftDescriptor',
  ...
  .addOptionalParam(
    'wordsDescriptor',
    'The `WordsDescriptor` contract address',
    '0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512',
    types.string,
  )
  .setAction(async ({ nftDescriptor, wordsDescriptor }, { ethers }) => {
    const descriptorFactory = await ethers.getContractFactory('WordsDescriptor', {
      libraries: {
        NFTDescriptor: nftDescriptor,
      },
    });
    const descriptorContract = descriptorFactory.attach(wordsDescriptor);
