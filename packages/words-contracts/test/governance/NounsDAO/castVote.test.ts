...
import {
  ...
  WordsDaoLogicV1Harness,
  WordsDaoLogicV1Harness__factory as WordsDaoLogicV1HarnessFactory,
} from '../../../typechain';
...
async function deployGovernor(
  ...
  tokenAddress: string,
): Promise<WordsDaoLogicV1Harness> {
  const { address: govDelegateAddress } = await new WordsDaoLogicV1HarnessFactory(
    deployer,
  ).deploy();
  const params = [
    address(0),
  ...
  const { address: _govDelegatorAddress } = await (
    await ethers.getContractFactory('WordsDAOProxy', deployer)
  ).deploy(...params);

  return WordsDaoLogicV1HarnessFactory.connect(_govDelegatorAddress, deployer);
}
...
let gov: WordsDaoLogicV1Harness;
let targets: string[];
let values: string[];
...
async function reset() {
  ...
  token = await deployWordsToken(signers.deployer);

  await populateDescriptor(
    WordsDescriptorFactory.connect(await token.descriptor(), signers.deployer),
  );
  ...
  describe('We must revert if:', () => {
    ...
    it("There does not exist a proposal with matching proposal id where the current block number is between the proposal's start block (exclusive) and end block (inclusive)", async () => {
      await expect(gov.castVote(proposalId, 1)).revertedWith(
        'WordsDAO::castVoteInternal: voting is closed',
      );
    });
    ...
    it('Such proposal already has an entry in its voters set matching the sender', async () => {
      ...
      await expect(gov.connect(account0).castVote(proposalId, 1)).revertedWith(
        'WordsDAO::castVoteInternal: voter already voted',
      );
    });
  });
