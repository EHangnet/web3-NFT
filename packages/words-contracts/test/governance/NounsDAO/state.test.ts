...
import {
  ...
  WordsDaoExecutorHarness,
  WordsDaoExecutorHarness__factory as WordsDaoExecutorHarnessFactory,
  WordsDaoImmutable,
  WordsDaoImmutable__factory as WordsDaoImmutableFactory,
} from '../../../typechain';
...
let gov: WordsDaoImmutable;
let timelock: WordsDaoExecutorHarness;
let delay: number;
...
async function makeProposal(
  ...
  timelock = await new WordsDaoExecutorHarnessFactory(deployer).deploy(deployer.address, delay);

  gov = await new WordsDaoImmutableFactory(deployer).deploy(
    timelock.address,
    token.address,
    address(0),
    deployer.address,
...
describe('WordsDAO#state/1', () => {
  ...
  it('Invalid for proposal not found', async () => {
    await makeProposal();
    await expect(gov.state(5)).revertedWith('WordsDAO::state: invalid proposal id');
  });
