...
async function reset(): Promise<void> {
  ...
  const { address: timelockAddress } = await new WordsDaoExecutorFactory(deployer).deploy(
    govDelegatorAddress,
    timelockDelay,
  );
...
describe('WordsDAO#inflationHandling', () => {
  before(async () => {
  ...
  it('returns quorum votes and proposal threshold based on Word total supply', async () => {
    // Total Supply = 40
  ...
  it('rejects if proposing below threshold', async () => {
    ...
    ).revertedWith('WordsDAO::propose: proposer votes below proposal threshold');
  });
