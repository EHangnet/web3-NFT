...
async function reset(): Promise<void> {
  ...
  // nonce 3: Deploy WordsDescriptor
  // nonce 4: Deploy WordsSeeder
  // nonce 5: Deploy WordsToken
  // nonce 6: Deploy WordsDAOProxy
  ...
  // Deploy WordsDAOExecutor with pre-computed Delegator address
  timelock = await new WordsDaoExecutorFactory(deployer).deploy(govDelegatorAddress, timelockDelay);
  const timelockAddress = timelock.address;

  // Deploy Delegate
  const { address: govDelegateAddress } = await new WordsDaoLogicV1Factory(deployer).deploy();

  // Deploy Words token
  token = await deployWordsToken(deployer);

  // Deploy Delegator
  await new WordsDaoProxyFactory(deployer).deploy(
    timelockAddress,
    token.address,
    vetoer.address,
    timelockAddress,
  ...
  gov = WordsDaoLogicV1Factory.connect(govDelegatorAddress, deployer);

  await populateDescriptor(WordsDescriptorFactory.connect(await token.descriptor(), deployer));
...
describe('WordsDAO#vetoing', () => {
  ...
  it('rejects setting a new vetoer when sender is not vetoer', async () => {
    await expect(gov.connect(account0)._setVetoer(account1.address)).revertedWith(
      'WordsDAO::_setVetoer: vetoer only',
    );
  });
  ...
  it('only vetoer can veto', async () => {
    ...
    await expect(gov.veto(proposalId)).revertedWith('WordsDAO::veto: only vetoer');
  });

  it('burns veto power correctly', async () => {
    // vetoer is still set
    expect(await gov.vetoer()).to.equal(vetoer.address);
    await expect(gov._burnVetoPower()).revertedWith('WordsDAO::_burnVetoPower: vetoer only');
    // burn
    ...
    await expect(gov.connect(vetoer).veto(proposalId)).revertedWith(
      'WordsDAO::veto: veto power burned',
    );
  });
  ...
  describe('vetoing works correctly for proposal state', async () => {
    ...
    it('Executed', async () => {
      ...
      await expect(gov.veto(proposalId)).revertedWith(
        'WordsDAO::veto: cannot veto executed proposal',
      );
    });
    it('Vetoed', async () => {
      await propose(account0);
