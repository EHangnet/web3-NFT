...
import { deployWordsToken, deployWeth, populateDescriptor } from './utils';

chai.use(solidity);
const { expect } = chai;

describe('WordsAuctionHouse', () => {
  ...
  async function deploy(deployer?: SignerWithAddress) {
    ...
    return upgrades.deployProxy(auctionHouseFactory, [
      ...
      DURATION,
    ]) as Promise<WordsAuctionHouse>;
  }

  before(async () => {
    [deployer, worddersDAO, bidderA, bidderB] = await ethers.getSigners();

    wordsToken = await deployWordsToken(deployer, worddersDAO.address, deployer.address);
    weth = await deployWeth(deployer);
    wordsAuctionHouse = await deploy(deployer);
  ...
  it('should revert if a second initialization is attempted', async () => {
    const tx = wordsAuctionHouse.initialize(
      wordsToken.address,
      weth.address,
      TIME_BUFFER,
      RESERVE_PRICE,
  ...
  it('should allow the worddersDAO to unpause the contract and create the first auction', async () => {
    const tx = await wordsAuctionHouse.unpause();
    await tx.wait();

    const auction = await wordsAuctionHouse.auction();
  ...
  it('should revert if a user creates a bid for an inactive auction', async () => {
    await (await wordsAuctionHouse.unpause()).wait();

    const { wordId } = await wordsAuctionHouse.auction();
    const tx = wordsAuctionHouse.connect(bidderA).createBid(wordId.add(1), {
      value: RESERVE_PRICE,
    });

    await expect(tx).to.be.revertedWith('Word not up for auction');
  });
  ...
  it('should revert if a user creates a bid for an expired auction', async () => {
    ...
    const { wordId } = await wordsAuctionHouse.auction();
    const tx = wordsAuctionHouse.connect(bidderA).createBid(wordId, {
      value: RESERVE_PRICE,
    });
  ...
  it('should revert if a user creates a bid less than the min bid increment percentage', async () => {
    ...
    const { wordId } = await wordsAuctionHouse.auction();
    await wordsAuctionHouse.connect(bidderA).createBid(wordId, {
      value: RESERVE_PRICE * 50,
    });
    const tx = wordsAuctionHouse.connect(bidderB).createBid(wordId, {
  ...
  it('should refund the previous bidder when the following user creates a bid', async () => {
    ...
    await wordsAuctionHouse.connect(bidderB).createBid(wordId, {
  ...
  it('should cap the maximum bid griefing cost at 30K gas + the cost to wrap and transfer WETH', async () => {
    ...
    const maliciousBid = await maliciousBidder
      .connect(bidderA)
      .bid(wordsAuctionHouse.address, wordId, {
        value: RESERVE_PRICE,
      });
    await maliciousBid.wait();

    const tx = await wordsAuctionHouse.connect(bidderB).createBid(wordId, {
      value: RESERVE_PRICE * 2,
      gasLimit: 1_000_000,
    });
  ...
  it('should emit an `AuctionBid` event on a successful bid', async () => {
    ...
    await expect(tx)
      .to.emit(wordsAuctionHouse, 'AuctionBid')
      .withArgs(wordId, bidderA.address, RESERVE_PRICE, false);
  });

  it('should emit an `AuctionExtended` event if the auction end time is within the time buffer', async () => {
    await (await wordsAuctionHouse.unpause()).wait();

    const { wordId, endTime } = await wordsAuctionHouse.auction();
    ...
    await expect(tx)
      .to.emit(wordsAuctionHouse, 'AuctionExtended')
      .withArgs(wordId, endTime.add(60 * 10));
  });
  ...
  it('should revert if auction settlement is attempted while the auction is still active', async () => {
    ...
    const tx = wordsAuctionHouse.connect(bidderA).settleCurrentAndCreateNewAuction();
  ...
  it('should emit `AuctionSettled` and `AuctionCreated` events if all conditions are met', async () => {
    ...
    await ethers.provider.send('evm_increaseTime', [60 * 60 * 25]); // Add 25 hours
    const tx = await wordsAuctionHouse.connect(bidderA).settleCurrentAndCreateNewAuction();
  ...
  it('should not create a new auction if the auction house is paused and unpaused while an auction is ongoing', async () => {
    ...
    await (await wordsAuctionHouse.pause()).wait();

    await (await wordsAuctionHouse.unpause()).wait();

    const { wordId } = await wordsAuctionHouse.auction();

    expect(wordId).to.equal(1);
  });
  ...
  it('should create a new auction if the auction house is paused and unpaused after an auction is settled', async () => {
    ...
    const settleTx = wordsAuctionHouse.connect(bidderA).settleAuction();

    await expect(settleTx)
      .to.emit(wordsAuctionHouse, 'AuctionSettled')
      .withArgs(wordId, bidderA.address, RESERVE_PRICE);

    const unpauseTx = await wordsAuctionHouse.unpause();
  ...
  it('should settle the current auction and pause the contract if the minter is updated while the auction house is unpaused', async () => {
    ...
    await wordsToken.setMinter(constants.AddressZero);

    await ethers.provider.send('evm_increaseTime', [60 * 60 * 25]); // Add 25 hours

    const settleTx = wordsAuctionHouse.connect(bidderA).settleCurrentAndCreateNewAuction();

    await expect(settleTx)
      .to.emit(wordsAuctionHouse, 'AuctionSettled')
      .withArgs(wordId, bidderA.address, RESERVE_PRICE);

    const paused = await wordsAuctionHouse.paused();

    expect(paused).to.equal(true);
  });

  it('should burn a Word on auction settlement if no bids are received', async () => {
    await (await wordsAuctionHouse.unpause()).wait();
    ...
    await expect(tx)
      .to.emit(wordsAuctionHouse, 'AuctionSettled')
      .withArgs(wordId, '0x0000000000000000000000000000000000000000', 0);
  });
});
