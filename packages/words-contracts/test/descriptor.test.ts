...
import { WordsDescriptor } from '../typechain';
import ImageData from '../files/image-data.json';
import { LongestPart } from './types';
import { deployWordsDescriptor, populateDescriptor } from './utils';
...
describe('WordsDescriptor', () => {
  let wordsDescriptor: WordsDescriptor;
  ...
  before(async () => {
    wordsDescriptor = await deployWordsDescriptor();
    ...
    await populateDescriptor(wordsDescriptor);
  });
  ...
  it('should generate valid token uri metadata when data uris are disabled', async () => {
    const BASE_URI = 'https://api.words.wtf/metadata/';

    await wordsDescriptor.setBaseURI(BASE_URI);
    await wordsDescriptor.toggleDataURIEnabled();

    const tokenUri = await wordsDescriptor.tokenURI(0, {
      background: 0,
  ...
  it('should generate valid token uri metadata when data uris are enabled', async () => {
    ...
    expect(name).to.equal('Word 0');
    expect(description).to.equal('Word 0 is a member of the Words DAO');
  ...
  it.skip('should generate valid token uri metadata for all supported parts when data uris are enabled', async () => {
    ...
    for (let i = 0; i < max; i++) {
      const tokenUri = await wordsDescriptor.tokenURI(i, {
      ...
      expect(name).to.equal(`Word ${i}`);
      expect(description).to.equal(`Word ${i} is a member of the Words DAO`);
      expect(image).to.not.be.undefined;
