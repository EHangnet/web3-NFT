...
import { WordSeed } from '../src/types';

const { bodies, accessories, heads, glasses } = images;

describe('@word/assets utils', () => {
  // Test against Word 116, created at block 13661786
  const NOUN116_ID = 116;
  const NOUN116_SEED: WordSeed = {
    background: 1,
    body: 28,
    accessory: 120,
    head: 95,
  ...
  describe('getPseudorandomPart', () => {
    it('should match WordsSeeder.sol implementation for a pseudorandomly chosen part', () => {
      const headShift = 144;
  ...
  describe('getWordSeedFromBlockHash', () => {
    it('should match WordsSeeder.sol implementation for generating a Word seed', () => {
      expect(getWordSeedFromBlockHash(NOUN116_ID, NOUN116_PREV_BLOCKHASH)).to.deep.equal(
        NOUN116_SEED,
      );
    });
  });
