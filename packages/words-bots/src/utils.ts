...
import { wordsTokenContract } from './clients';
...
export function formatAuctionStartedTweetText(auctionId: number) {
 ...
 An auction has started for Word #${auctionId}
 Learn more at https://words.wtf`;
}

/**
 * Get the formatted text for a new bid.
 * @param id The auction/word id
...
export async function formatBidMessageText(id: number, bid: Bid) {
  ...
  return `Word ${id} has received a bid of Ξ${ethers.utils.formatEther(bid.amount)} from ${bidder}`;
}
...
export function getAuctionEndingSoonTweetText() {
  return `This auction is ending soon! Bid now at https://words.wtf`;
}

/**
 * Get the PNG buffer data of a Word
 * @param tokenId The ERC721 token id
 * @returns The png buffer of the Word or undefined
 */
export async function getWordPngBuffer(tokenId: string): Promise<Buffer | undefined> {
  const dataURI = await tryF(() => wordsTokenContract.dataURI(tokenId));
  if (isError(dataURI)) {
