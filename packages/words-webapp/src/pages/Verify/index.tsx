...
import { wordsIndex } from '../../wrappers/subgraph';
...
const VerifyPage: React.FC<VerifyPageProp> = props => {
  ...
  const { data } = useQuery(wordsIndex());
  ...
  const extractOwnedWordIdsFromWordsIndex = (owner: string | undefined, wordsIndex: any) =>
    R.pipe(
      (words: any) =>
        words.filter((word: any) =>
          !owner
            ? false
            : (word.owner.id as string)
                .toLocaleLowerCase()
                .localeCompare(owner.toLocaleLowerCase()) === 0,
        ),
      R.map((word: any) => Number(word.id)),
      R.sort((a: number, b: number) => a - b),
    )(wordsIndex.words);

  const loadingContent = () => <div className={classes.loadingContent}>loading your Words...</div>;

  useEffect(() => {
    if (!data) return;
    const initialMessage = (words: number[]) =>
      [
        activeAccount ? `I am ${activeAccount}` : undefined,
        words.length > 0
          ? ` and I own Word${words.length > 1 ? 's' : ''} ${words.join(', ')}`
          : undefined,
      ]
        .filter((part: string | undefined) => part)
        .join(' ');
    setMessageToSign(initialMessage(extractOwnedWordIdsFromWordsIndex(activeAccount, data)));
  }, [data, activeAccount]);
