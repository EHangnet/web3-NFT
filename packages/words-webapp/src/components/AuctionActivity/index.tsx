...
const openEtherscanBidHistory = () => {
  const url = buildEtherscanAddressLink(config.addresses.wordsAuctionHouseProxy);
  window.open(url);
};
...
const AuctionActivity: React.FC<AuctionActivityProps> = (props: AuctionActivityProps) => {
  ...
  const bidHistoryTitle = (
    <h1>
      Word {auction && auction.wordId.toString()}
      <br /> Bid History
    </h1>
  );
      ...
      {showBidHistoryModal && (
        ...
        >
          <Modal.Header closeButton className={classes.modalHeader}>
            <div className={classes.modalHeaderWordImgWrapper}>
              <StandaloneWord wordId={auction && auction.wordId} />
            </div>
            <Modal.Title className={classes.modalTitleWrapper}>{bidHistoryTitle}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <BidHistory auctionId={auction.wordId.toString()} max={9999} />
          </Modal.Body>
        </Modal>
      )}
      ...
      <AuctionActivityWrapper>
        <div className={classes.informationRow}>
          <Row className={classes.activityRow}>
            ...
            <AuctionTitleAndNavWrapper>
              <AuctionActivityWordTitle wordId={auction.wordId} />
              {displayGraphDepComps && (
                <AuctionNavigation
        ...
        <Row className={classes.activityRow}>
          <Col lg={12}>
            {displayGraphDepComps && (
              <BidHistory
                auctionId={auction.wordId.toString()}
                max={3}
