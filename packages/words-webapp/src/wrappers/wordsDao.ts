import { WordsDAOABI, WordsDaoLogicV1Factory } from '@words/sdk';
...
const abi = new utils.Interface(WordsDAOABI);
const wordsDaoContract = new WordsDaoLogicV1Factory().attach(config.addresses.wordsDAOProxy);
const proposalCreatedFilter = wordsDaoContract.filters?.ProposalCreated(
  null,
  null,
  null,
  null,
...
export const useHasVotedOnProposal = (proposalId: string | undefined): boolean => {
  ...
  const [receipt] =
    useContractCall<[any]>({
      abi,
      address: wordsDaoContract.address,
      method: 'getReceipt',
...
const useVotingDelay = (wordsDao: string): number | undefined => {
  const [blockDelay] =
    useContractCall<[EthersBN]>({
      abi,
      address: wordsDao,
...
export const useAllProposals = (): ProposalData => {
  ...
  const votingDelay = useVotingDelay(wordsDaoContract.address);
...
export const usePropose = () => {
  const { send: propose, state: proposeState } = useContractFunction(wordsDaoContract, 'propose');
