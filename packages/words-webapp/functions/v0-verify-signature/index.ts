...
import { isWordDelegate, isWordOwner, wordsQuery } from '../theGraph';
...
const handler: Handler = async (event, context) => {
  ...
  if (event.queryStringParameters.fetchParticipation && validSignature) {
    const normalizedWords = await wordsQuery();
    participantData = {
      isWordDelegate: isWordDelegate(signer, normalizedWords),
      isWordOwner: isWordOwner(signer, normalizedWords),
    };
  }
